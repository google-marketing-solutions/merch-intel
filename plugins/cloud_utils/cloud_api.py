...
class CloudApiUtils(object):
  ...
  def __init__(
      self, project_id: str, service_account_key_file: str = None
  ) -> None:
    ...
    Args:
      ...
      service_account_key_file: Optional. File containing service account key.
        ...
        ways to create service accounts: 1. Use `create_service_account_key`
        method from `cloud_auth` module. 2. Use `gcloud` command line utility as
        documented here -
  ...
  def enable_apis(self, apis: List[str]) -> None:
    ...
    try:
      ...
      raise Error('Error occurred while enabling Cloud APIs.') from error
...
def post_request(session: AuthorizedSession, url: str, data: Any) -> None:
  ...
  try:
    ...
    logging.exception(
        'HTTPError "%s" "%s": post_request failed',
        error.response.status_code,
        error.response.reason,
    )
    raise Error(
        'HTTPError {} {}: post_request failed.'.format(
...
def disable_api(session: AuthorizedSession, project_id: str, api: str) -> None:
  ...
  """
  disable_api_url = '{}/{}/services/{}:disable'.format(
      _SERVICE_URL, project_id, api
  )
  logging.info(
      'Disabling following API for "%s" project : "%s".', project_id, api
  )
  post_request(session, disable_api_url, {'disableDependentServices': True})


def is_api_enabled(
    session: AuthorizedSession, project_id: str, api: str
) -> bool:
