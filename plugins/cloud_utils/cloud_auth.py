...
def build_service_client(
    service_name: str, service_account_key_file: str = None
...
def _get_service_account_email(
    project_id: str, service_account_name: str
) -> str:
...
def create_service_account(
    project_id: str, service_account_name: str, role_name: str, file_name: str
) -> Dict[str, Any]:
  ...
  service_account_details = get_service_account(
      project_id, service_account_name
  )
  if service_account_details:
    return service_account_details
  logging.info(
      'Creating "%s" service account in "%s" project',
  ...
  request = _get_service_account_client().create(
      ...
      body={
          ...
          'serviceAccount': {'displayName': service_account_name.upper()},
      },
  )
...
def get_service_account(
  ...
  try:
    logging.info(
        'Retrieving "%s" service account in "%s" project',
        service_account_name,
        project_id,
    )
    name = (
        'projects/{p}/serviceAccounts/{s}@{p}.iam.gserviceaccount.com'.format(
            p=project_id, s=service_account_name
        )
    )
    service_account_details = (
        _get_service_account_client().get(name=name).execute()
    )
    return service_account_details
    ...
    raise Error('Error occurred while retrieving service account.') from error


def create_service_account_key(
    project_id: str, service_account_name: str, file_name: str
) -> None:
...
def _create_service_account_key(
    project_id: str, service_account_name: str, file_object: Any
) -> None:
...
def set_service_account_role(
    project_id, service_account_name, role_name
) -> None:
  ...
  """
  logging.info(
      'Adding "%s" role to "%s" service account in "%s" project',
      role_name,
      service_account_name,
      project_id,
  )
  ...
  request = (
      _get_resource_manager_client()
      .projects()
      .setIamPolicy(resource=project_id, body=set_iam_policy_request_body)
  )
